...
if(typeof debug_mode === 'undefined')
	window.debug_mode = false;//调试模式，上线后必须把此参数改为false！
if(typeof enable_animate === 'undefined')
	window.enable_animate = true;//开启动画

if(typeof iflytekMp === 'undefined'){
...
var KEY_VIDEO_BACK  =  0x0018;  // 视频页回退
...
function log(msg){
	...
	msg = window.location.pathname + "--->>>" + msg;
	//创建日志显示面板
	var NAME = "console";
	var panel = doc.getElementById(NAME);
	if (!panel) {
	...
	panel && panel.appendChild(node);

	//超过屏幕高度自动清理
	panel && panel.offsetHeight >= doc.body.clientHeight && (panel.innerHTML = "");
	//清理后将最后一条重新添加
	panel && panel.appendChild(node);
}
	...
	return {
		...
		fullscreenPlay: function(url)
		{
			if( is_ott )
			{
				...
				mp.initMediaPlayer(url,0, 0 , 1280,720);
				mp.setWebViewBackgroundAlpha(0);
				mp.setUseTopOrBottomSurfaceView(0);
			}
			else
			{
		...
		{
			//这个toFixed将来要删除，因为暂时没用到这个方法，所以先不管，update by lxa 20140812
		...
		getRate:function()
		{
			...
			try
			{
				var current=mp.getCurrentPlayTime();//当前秒数
				var all=mp.getMediaDuration();//总的秒数
				return (current/all);
				//return (current/all).toFixed(2);//保留2位小数,创维不支持toFixed()，update by lxa 20140812
			}
			catch(e)
			{
				return 0;
